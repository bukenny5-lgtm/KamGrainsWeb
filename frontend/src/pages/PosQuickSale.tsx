...
import { createPosSale, getAvailablePaymentChannels, getParties, getPosProducts, getTaxSettings } from "@/api/client";
...
export default function PosQuickSale() {
  ...
  const paymentChannelsQuery = useQuery({
    queryKey: ["pos-payment-channels", paymentMethod, operating.currentBranch?.branch_id, locationId],
    queryFn: () => getAvailablePaymentChannels({ payment_method: paymentMethod, branch_id: operating.currentBranch?.branch_id, location_id: locationId }),
    enabled: paymentMethod !== "CREDIT" && Boolean(operating.currentBranch?.branch_id && locationId),
  });
  const paymentChannels = (paymentChannelsQuery.data?.data || []) as Array<Record<string, any>>;
  const selectedPaymentChannel = paymentChannels.find((channel) => channel.api_payment_channel_id === paymentChannelId) || (paymentChannels.length === 1 ? paymentChannels[0] : null);
  ...
  function completeSale() {
    if (!cart.length || hasQuantityErrors || (paymentMethod === "CREDIT" && !customerId) || (paymentMethod === "CASH" && Number(amountTendered || 0) < total) || (selectedPaymentChannel && paymentMethod !== "CASH" && !paymentReference.trim())) return;
    saleMutation.mutate({
      location_id: locationId,
      customer_id: customerId || null,
      payment_method: paymentMethod,
      payment_channel_id: selectedPaymentChannel?.api_payment_channel_id || null,
  ...
  return (
    <div className="space-y-6">
      ...
      {paymentMethod !== "CASH" && paymentMethod !== "CREDIT" && <section className="rounded-2xl border bg-white p-5 shadow-sm"><h2 className="mb-3 text-lg font-semibold">Manual Payment Confirmation</h2><label className="block text-sm">Configured channel<select className="mt-1 w-full rounded-lg border px-3 py-2" value={paymentChannelId || (paymentChannels.length === 1 ? paymentChannels[0].api_payment_channel_id : "")} onChange={(event) => setPaymentChannelId(event.target.value)}><option value="">{paymentChannelsQuery.isLoading ? "Loading channels..." : paymentChannels.length ? "Select channel" : "No active manual channel configured"}</option>{paymentChannels.map((channel) => <option key={channel.api_payment_channel_id} value={channel.api_payment_channel_id}>{channel.channel_name} · {channel.provider_name}</option>)}</select></label>{selectedPaymentChannel && <div className="mt-3 rounded border bg-slate-50 p-3 text-xs"><b>Manual Confirmation</b><div>Amount: {money(total)}</div><div>Pay to: {selectedPaymentChannel.merchant_code || selectedPaymentChannel.wallet_number_masked || selectedPaymentChannel.account_number_masked || selectedPaymentChannel.terminal_name || selectedPaymentChannel.channel_name}</div></div>}<input className="mt-3 w-full rounded-lg border px-3 py-2" type="text" placeholder={paymentMethod === "CARD" ? "Approval / terminal reference" : "External transaction / transfer reference"} value={paymentReference} onChange={(event) => setPaymentReference(event.target.value)} />{selectedPaymentChannel && !paymentReference.trim() && <p className="mt-1 text-xs text-red-600">Manual confirmation reference is required.</p>}</section>}
